...
/********************
 *   A simple Comp farming strategy from leveraged lending of want.
...
contract GenericLevCompStratV2 is BaseStrategyV0_1_1, DydxFlashloanBase, ICallee, FlashLoanReceiverBase {
    ...
    constructor(address _vault, address _cToken) public BaseStrategyV0_1_1(_vault) FlashLoanReceiverBase(AAVE_LENDING) {
    ...
    function prepareReturn() internal override {
        ...
        if (balance > debt) {
            ...
            if (profit >= daiBalance) {
                ...
                //some want is not profit and needs to pay off our interest
    ...
    function _withdrawSome(uint256 _amount, bool _useBackup) internal returns (bool notAll) {
        ...
        if (depositBalance.sub(AmountNeeded) < _amount) {
            cToken.redeemUnderlying(depositBalance.sub(AmountNeeded));
        } else {
