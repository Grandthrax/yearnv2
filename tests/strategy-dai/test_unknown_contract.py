...
def test_unknown_2(
    ...
    # deploy vault
    vault = whaleU.deploy(Vault, uni_wethwbtc, whaleU, whaleU, "ss", "sss")
...
def test_unknown_1(
    web3, StrategyUniswapPairPickle, Vault, chain, uni_wethwbtc, whaleU, accounts
):
    ...
    for i in range(15):
        ...
        if action == 1:
            vault.withdraw(vault.balanceOf(whale) * 0.999, {"from": whale})
        elif action == 2:
            vault.withdraw(vault.balanceOf(whale) * 0.999, {"from": whale})
        # elif action == 3:
